...
#if __has_include("hook/vkEnumerateInstanceExtensionPropertiesForwarderHook.hpp")
    #include "hook/vkEnumerateInstanceExtensionPropertiesForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateInstanceExtensionProperties_entry(const char * pLayerName, uint32_t * pPropertyCount,
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::EnumerateInstanceExtensionProperties>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(pLayerName, pPropertyCount, pProperties); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.pLayerName = pLayerName, .pPropertyCount = pPropertyCount, .pProperties = pProperties};
