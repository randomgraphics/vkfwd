...
#if __has_include("hook/vkCreateGraphicsPipelinesForwarderHook.hpp")
    #include "hook/vkCreateGraphicsPipelinesForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateGraphicsPipelines_entry(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount,
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::CreateGraphicsPipelines>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, pipelineCache, createInfoCount, pCreateInfos, pAllocator, pPipelines); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device          = device,
    ...
    if (pPipelines && response.pPipelines && response.pPipelines != pPipelines && response.createInfoCount) {
        std::copy_n(response.pPipelines, response.createInfoCount, pPipelines);
    }
