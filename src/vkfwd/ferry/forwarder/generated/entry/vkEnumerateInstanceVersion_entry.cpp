...
#if __has_include("hook/vkEnumerateInstanceVersionForwarderHook.hpp")
    #include "hook/vkEnumerateInstanceVersionForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateInstanceVersion_entry(uint32_t * pApiVersion) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::EnumerateInstanceVersion>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(pApiVersion); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.pApiVersion = pApiVersion};
