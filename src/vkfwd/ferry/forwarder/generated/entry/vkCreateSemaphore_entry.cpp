...
#if __has_include("hook/vkCreateSemaphoreForwarderHook.hpp")
    #include "hook/vkCreateSemaphoreForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreateSemaphore_entry(VkDevice device, const VkSemaphoreCreateInfo * pCreateInfo, const VkAllocationCallbacks * pAllocator,
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::CreateSemaphore>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, pCreateInfo, pAllocator, pSemaphore); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .pCreateInfo = pCreateInfo, .pAllocator = pAllocator, .pSemaphore = pSemaphore};
    ...
    if (pSemaphore && response.pSemaphore && response.pSemaphore != pSemaphore) { *pSemaphore = *response.pSemaphore; }
