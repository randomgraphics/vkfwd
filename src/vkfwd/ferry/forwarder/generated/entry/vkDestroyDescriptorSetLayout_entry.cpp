...
#if __has_include("hook/vkDestroyDescriptorSetLayoutForwarderHook.hpp")
    #include "hook/vkDestroyDescriptorSetLayoutForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkDestroyDescriptorSetLayout_entry(VkDevice device, VkDescriptorSetLayout descriptorSetLayout,
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::DestroyDescriptorSetLayout>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, descriptorSetLayout, pAllocator); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .descriptorSetLayout = descriptorSetLayout, .pAllocator = pAllocator};
