...
#if __has_include("hook/vkFreeMemoryForwarderHook.hpp")
    #include "hook/vkFreeMemoryForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkFreeMemory_entry(VkDevice device, VkDeviceMemory memory, const VkAllocationCallbacks * pAllocator) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::FreeMemory>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, memory, pAllocator); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .memory = memory, .pAllocator = pAllocator};
