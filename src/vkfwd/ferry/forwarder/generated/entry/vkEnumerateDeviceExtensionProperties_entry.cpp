...
#if __has_include("hook/vkEnumerateDeviceExtensionPropertiesForwarderHook.hpp")
    #include "hook/vkEnumerateDeviceExtensionPropertiesForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateDeviceExtensionProperties_entry(VkPhysicalDevice physicalDevice, const char * pLayerName, uint32_t * pPropertyCount,
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::EnumerateDeviceExtensionProperties>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(physicalDevice, pLayerName, pPropertyCount, pProperties); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.physicalDevice = physicalDevice, .pLayerName = pLayerName, .pPropertyCount = pPropertyCount, .pProperties = pProperties};
    ...
    if (pPropertyCount && response.pPropertyCount && response.pPropertyCount != pPropertyCount) { *pPropertyCount = *response.pPropertyCount; }
    if (pProperties && response.pProperties && response.pProperties != pProperties && response.pPropertyCount) {
        std::copy_n(response.pProperties, *response.pPropertyCount, pProperties);
    }
