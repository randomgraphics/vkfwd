...
#if __has_include("hook/vkEnumerateInstanceLayerPropertiesForwarderHook.hpp")
    #include "hook/vkEnumerateInstanceLayerPropertiesForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkEnumerateInstanceLayerProperties_entry(uint32_t * pPropertyCount, VkLayerProperties * pProperties) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::EnumerateInstanceLayerProperties>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(pPropertyCount, pProperties); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.pPropertyCount = pPropertyCount, .pProperties = pProperties};
