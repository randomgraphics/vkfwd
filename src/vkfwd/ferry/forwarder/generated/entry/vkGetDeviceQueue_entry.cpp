...
#if __has_include("hook/vkGetDeviceQueueForwarderHook.hpp")
    #include "hook/vkGetDeviceQueueForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkGetDeviceQueue_entry(VkDevice device, uint32_t queueFamilyIndex, uint32_t queueIndex, VkQueue * pQueue) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::GetDeviceQueue>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, queueFamilyIndex, queueIndex, pQueue); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .queueFamilyIndex = queueFamilyIndex, .queueIndex = queueIndex, .pQueue = pQueue};
    ...
    if (pQueue && response.pQueue && response.pQueue != pQueue) { *pQueue = *response.pQueue; }
