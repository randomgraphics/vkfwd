...
#if __has_include("hook/vkAllocateMemoryForwarderHook.hpp")
    #include "hook/vkAllocateMemoryForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkAllocateMemory_entry(VkDevice device, const VkMemoryAllocateInfo * pAllocateInfo, const VkAllocationCallbacks * pAllocator,
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::AllocateMemory>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, pAllocateInfo, pAllocator, pMemory); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .pAllocateInfo = pAllocateInfo, .pAllocator = pAllocator, .pMemory = pMemory};
    ...
    if (pMemory && response.pMemory && response.pMemory != pMemory) { *pMemory = *response.pMemory; }
