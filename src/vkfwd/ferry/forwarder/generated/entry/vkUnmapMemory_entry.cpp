...
#if __has_include("hook/vkUnmapMemoryForwarderHook.hpp")
    #include "hook/vkUnmapMemoryForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkUnmapMemory_entry(VkDevice device, VkDeviceMemory memory) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::UnmapMemory>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, memory); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .memory = memory};
