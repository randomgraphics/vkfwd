...
#if __has_include("hook/vkDestroyBufferForwarderHook.hpp")
    #include "hook/vkDestroyBufferForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkDestroyBuffer_entry(VkDevice device, VkBuffer buffer, const VkAllocationCallbacks * pAllocator) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::DestroyBuffer>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, buffer, pAllocator); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .buffer = buffer, .pAllocator = pAllocator};
