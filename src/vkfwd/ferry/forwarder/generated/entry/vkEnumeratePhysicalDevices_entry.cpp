...
#if __has_include("hook/vkEnumeratePhysicalDevicesForwarderHook.hpp")
    #include "hook/vkEnumeratePhysicalDevicesForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkEnumeratePhysicalDevices_entry(VkInstance instance, uint32_t * pPhysicalDeviceCount, VkPhysicalDevice * pPhysicalDevices) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::EnumeratePhysicalDevices>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(instance, pPhysicalDeviceCount, pPhysicalDevices); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.instance = instance, .pPhysicalDeviceCount = pPhysicalDeviceCount, .pPhysicalDevices = pPhysicalDevices};
    ...
    if (pPhysicalDeviceCount && response.pPhysicalDeviceCount && response.pPhysicalDeviceCount != pPhysicalDeviceCount) {
        *pPhysicalDeviceCount = *response.pPhysicalDeviceCount;
    }
    if (pPhysicalDevices && response.pPhysicalDevices && response.pPhysicalDevices != pPhysicalDevices && response.pPhysicalDeviceCount) {
        std::copy_n(response.pPhysicalDevices, *response.pPhysicalDeviceCount, pPhysicalDevices);
    }
