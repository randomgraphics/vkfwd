...
#if __has_include("hook/vkDestroyPipelineForwarderHook.hpp")
    #include "hook/vkDestroyPipelineForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkDestroyPipeline_entry(VkDevice device, VkPipeline pipeline, const VkAllocationCallbacks * pAllocator) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::DestroyPipeline>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, pipeline, pAllocator); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .pipeline = pipeline, .pAllocator = pAllocator};
