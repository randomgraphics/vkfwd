...
#if __has_include("hook/vkDestroyRenderPassForwarderHook.hpp")
    #include "hook/vkDestroyRenderPassForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkDestroyRenderPass_entry(VkDevice device, VkRenderPass renderPass, const VkAllocationCallbacks * pAllocator) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::DestroyRenderPass>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, renderPass, pAllocator); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .renderPass = renderPass, .pAllocator = pAllocator};
