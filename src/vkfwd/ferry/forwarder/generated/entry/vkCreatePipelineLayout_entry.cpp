...
#if __has_include("hook/vkCreatePipelineLayoutForwarderHook.hpp")
    #include "hook/vkCreatePipelineLayoutForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkCreatePipelineLayout_entry(VkDevice device, const VkPipelineLayoutCreateInfo * pCreateInfo,
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::CreatePipelineLayout>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, pCreateInfo, pAllocator, pPipelineLayout); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .pCreateInfo = pCreateInfo, .pAllocator = pAllocator, .pPipelineLayout = pPipelineLayout};
    ...
    if (pPipelineLayout && response.pPipelineLayout && response.pPipelineLayout != pPipelineLayout) { *pPipelineLayout = *response.pPipelineLayout; }
