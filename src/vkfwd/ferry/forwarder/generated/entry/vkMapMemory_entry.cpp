...
VKAPI_ATTR VkResult VKAPI_CALL vkMapMemory_entry(VkDevice device, VkDeviceMemory memory, VkDeviceSize offset, VkDeviceSize size, VkMemoryMapFlags flags,
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::MapMemory>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, memory, offset, size, flags, ppData); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .memory = memory, .offset = offset, .size = size, .flags = flags, .ppData = ppData};
    ...
    if (ppData && response.ppData && response.ppData != ppData) { *ppData = *response.ppData; }
