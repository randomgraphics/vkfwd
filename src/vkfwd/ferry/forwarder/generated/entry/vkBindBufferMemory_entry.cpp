...
#if __has_include("hook/vkBindBufferMemoryForwarderHook.hpp")
    #include "hook/vkBindBufferMemoryForwarderHook.hpp"
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL vkBindBufferMemory_entry(VkDevice device, VkBuffer buffer, VkDeviceMemory memory, VkDeviceSize memoryOffset) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::BindBufferMemory>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, buffer, memory, memoryOffset); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .buffer = buffer, .memory = memory, .memoryOffset = memoryOffset};
