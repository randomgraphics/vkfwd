...
#if __has_include("hook/vkGetPhysicalDeviceMemoryPropertiesForwarderHook.hpp")
    #include "hook/vkGetPhysicalDeviceMemoryPropertiesForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkGetPhysicalDeviceMemoryProperties_entry(VkPhysicalDevice physicalDevice, VkPhysicalDeviceMemoryProperties * pMemoryProperties) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::GetPhysicalDeviceMemoryProperties>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(physicalDevice, pMemoryProperties); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.physicalDevice = physicalDevice, .pMemoryProperties = pMemoryProperties};
    ...
    if (pMemoryProperties && response.pMemoryProperties && response.pMemoryProperties != pMemoryProperties) {
        *pMemoryProperties = *response.pMemoryProperties;
    }
