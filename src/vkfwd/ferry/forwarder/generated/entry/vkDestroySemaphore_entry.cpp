...
#if __has_include("hook/vkDestroySemaphoreForwarderHook.hpp")
    #include "hook/vkDestroySemaphoreForwarderHook.hpp"
#endif
...
VKAPI_ATTR void VKAPI_CALL vkDestroySemaphore_entry(VkDevice device, VkSemaphore semaphore, const VkAllocationCallbacks * pAllocator) {
    ...
    using Hooks   = ::vkfwd::forwarder::manual::CommandHooks<::vkfwd::generated::CommandId::DestroySemaphore>;

    if constexpr (Hooks::before_pack_enabled) { Hooks::before_pack(device, semaphore, pAllocator); }

    auto &              forwarder = ::vkfwd::Forwarder::instance();
    Command::Parameters parameters {.device = device, .semaphore = semaphore, .pAllocator = pAllocator};
