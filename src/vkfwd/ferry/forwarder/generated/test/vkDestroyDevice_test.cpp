...
TEST_CASE("vkDestroyDevice generated forwarder packs parameters when flushed") {
    auto & expected = scenario();
    install_pack_unpack_channel(handle_flush);

    vkfwd::forwarder::generated::vkDestroyDevice(expected.device, &expected.allocator);
