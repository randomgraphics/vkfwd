...
TEST_CASE("pNext chain packing accepts null and supported chains") {
    ...
    CHECK(pack_pnext_chain(nullptr, stream, packed) == VK_SUCCESS);
    CHECK(packed.offset == 0);

    auto tail = make_vulkan11_features();
    auto head = make_features2(&tail);

    CHECK(pack_pnext_chain(&head, stream, packed) == VK_SUCCESS);
    CHECK(stream.size() > 0);
}
...
TEST_CASE("pNext chain packing rejects unsupported structure types") {
    ...
    CHECK(pack_pnext_chain(&unsupported, stream, packed) == VK_ERROR_UNKNOWN);
    CHECK(packed.offset == 0);

    auto head = make_features2(&unsupported);
    CHECK(pack_pnext_chain(&head, stream, packed) == VK_ERROR_UNKNOWN);
    CHECK(packed.offset == 0);
}
...
TEST_CASE("pNext chain packing rejects cycles before copying payload") {
    ...
    CHECK(pack_pnext_chain(&self_cycle, stream, packed) == VK_ERROR_UNKNOWN);
    CHECK(packed.offset == 0);
    ...
    CHECK(pack_pnext_chain(&first, stream, packed) == VK_ERROR_UNKNOWN);
    CHECK(packed.offset == 0);
}
...
TEST_CASE("pNext chain packing rejects chains beyond the validation depth limit") {
    ...
    CHECK(pack_pnext_chain(chain.data(), stream, packed) == VK_ERROR_UNKNOWN);
    CHECK(packed.offset == 0);
}
...
TEST_CASE("pNext chain packing rejects unreadable node pointers without crashing") {
    ...
    CHECK(pack_pnext_chain(invalid_application_pointer(), stream, packed) == VK_ERROR_UNKNOWN);
    CHECK(packed.offset == 0);
...
TEST_CASE("packed pNext unpacking rejects corrupt stream references") {
    CommandStream stream;
    auto          empty_view = stream.at(0, 1);
