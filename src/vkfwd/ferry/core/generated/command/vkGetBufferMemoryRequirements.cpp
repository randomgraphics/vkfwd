...
namespace vkfwd::generated::commands::vkGetBufferMemoryRequirements {
namespace {
...
VkResult Command::pack_parameters(CommandStream & stream, const Parameters & parameters) {
    using Hooks = ::vkfwd::manual::CommandHooks<CommandId::GetBufferMemoryRequirements>;
    if constexpr (Hooks::before_pack_enabled) {
        ...
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::GetBufferMemoryRequirements, 1, hook_parameters, range, packed_parameters);
        ...
        status = pack_output_pointer(hook_parameters.pMemoryRequirements, stream, payload_offset + offsetof(Parameters, pMemoryRequirements),
                                     packed_parameters->pMemoryRequirements);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        Parameters * packed_parameters = nullptr;
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::GetBufferMemoryRequirements, 1, parameters, range, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = range.offset + command_payload_offset<Parameters>();

        status = pack_output_pointer(parameters.pMemoryRequirements, stream, payload_offset + offsetof(Parameters, pMemoryRequirements),
                                     packed_parameters->pMemoryRequirements);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = finalize_command_chunk(stream, range);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::unpack_parameters(SafeArrayView<std::uint8_t> & view, const Parameters ** parameters) {
    ...
    Parameters * packed_parameters = nullptr;
    VkResult     status            = unpack_command_chunk(view, CommandId::GetBufferMemoryRequirements, 1, &packed_parameters);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = recover_command_pointer(packed_parameters->pMemoryRequirements, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::pack_response(CommandStream & stream, const Response & response) {
    ...
    Range      range;
    VkResult   status = append_command_chunk(stream, CommandId::GetBufferMemoryRequirements, 1, response, range, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    const std::size_t payload_offset = range.offset + command_payload_offset<Response>();

    status = pack_output_pointer(response.pMemoryRequirements, stream, payload_offset + offsetof(Response, pMemoryRequirements),
                                 packed_response->pMemoryRequirements);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = finalize_command_chunk(stream, range);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    return VK_SUCCESS;
...
VkResult Command::unpack_response(SafeArrayView<std::uint8_t> & view, const Response ** response) {
    Response * packed_response = nullptr;
    VkResult   status          = unpack_command_chunk(view, CommandId::GetBufferMemoryRequirements, 1, &packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }

    status = recover_command_pointer(packed_response->pMemoryRequirements, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
