...
namespace vkfwd::generated::commands::vkCreateDescriptorSetLayout {
namespace {
...
VkResult Command::pack_parameters(CommandStream & stream, const Parameters & parameters) {
    using Hooks = ::vkfwd::manual::CommandHooks<CommandId::CreateDescriptorSetLayout>;
    if constexpr (Hooks::before_pack_enabled) {
        ...
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::CreateDescriptorSetLayout, 1, hook_parameters, range, packed_parameters);
        ...
        status = structure::pack_VkDescriptorSetLayoutCreateInfo(hook_parameters.pCreateInfo, stream, packed_pCreateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_pointer(hook_parameters.pSetLayout, stream, payload_offset + offsetof(Parameters, pSetLayout), packed_parameters->pSetLayout);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        Parameters * packed_parameters = nullptr;
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::CreateDescriptorSetLayout, 1, parameters, range, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = range.offset + command_payload_offset<Parameters>();

        structure::PackedStruct packed_pCreateInfo;
        status = structure::pack_VkDescriptorSetLayoutCreateInfo(parameters.pCreateInfo, stream, packed_pCreateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pCreateInfo, payload_offset + offsetof(Parameters, pCreateInfo), packed_pCreateInfo.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_pointer(parameters.pSetLayout, stream, payload_offset + offsetof(Parameters, pSetLayout), packed_parameters->pSetLayout);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = finalize_command_chunk(stream, range);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::unpack_parameters(SafeArrayView<std::uint8_t> & view, const Parameters ** parameters) {
    ...
    Parameters * packed_parameters = nullptr;
    VkResult     status            = unpack_command_chunk(view, CommandId::CreateDescriptorSetLayout, 1, &packed_parameters);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    ...
    if (packed_parameters->pCreateInfo) {
        ...
        const VkDescriptorSetLayoutCreateInfo * ignored_pCreateInfo = nullptr;
        status                                                      = structure::unpack_VkDescriptorSetLayoutCreateInfo(child_view, &ignored_pCreateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
    }
    status = recover_command_pointer(packed_parameters->pAllocator, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = recover_command_pointer(packed_parameters->pSetLayout, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::pack_response(CommandStream & stream, const Response & response) {
    ...
    Range      range;
    VkResult   status = append_command_chunk(stream, CommandId::CreateDescriptorSetLayout, 1, response, range, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    const std::size_t payload_offset = range.offset + command_payload_offset<Response>();

    status = pack_output_pointer(response.pSetLayout, stream, payload_offset + offsetof(Response, pSetLayout), packed_response->pSetLayout);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = finalize_command_chunk(stream, range);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    return VK_SUCCESS;
...
VkResult Command::unpack_response(SafeArrayView<std::uint8_t> & view, const Response ** response) {
    Response * packed_response = nullptr;
    VkResult   status          = unpack_command_chunk(view, CommandId::CreateDescriptorSetLayout, 1, &packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }

    status = recover_command_pointer(packed_response->pSetLayout, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
