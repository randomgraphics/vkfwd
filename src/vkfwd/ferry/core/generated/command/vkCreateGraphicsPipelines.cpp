...
namespace vkfwd::generated::commands::vkCreateGraphicsPipelines {
namespace {
...
VkResult Command::pack_parameters(CommandStream & stream, const Parameters & parameters) {
    using Hooks = ::vkfwd::manual::CommandHooks<CommandId::CreateGraphicsPipelines>;
    if constexpr (Hooks::before_pack_enabled) {
        ...
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::CreateGraphicsPipelines, 1, hook_parameters, range, packed_parameters);
        ...
        structure::PackedStruct packed_pCreateInfos;
        status = structure::pack_array_VkGraphicsPipelineCreateInfo(hook_parameters.pCreateInfos, hook_parameters.createInfoCount, stream, packed_pCreateInfos);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pCreateInfos, payload_offset + offsetof(Parameters, pCreateInfos), packed_pCreateInfos.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_array(hook_parameters.pPipelines, hook_parameters.createInfoCount, stream, payload_offset + offsetof(Parameters, pPipelines),
                                   packed_parameters->pPipelines);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        Parameters * packed_parameters = nullptr;
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::CreateGraphicsPipelines, 1, parameters, range, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = range.offset + command_payload_offset<Parameters>();

        structure::PackedStruct packed_pCreateInfos;
        status = structure::pack_array_VkGraphicsPipelineCreateInfo(parameters.pCreateInfos, parameters.createInfoCount, stream, packed_pCreateInfos);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pCreateInfos, payload_offset + offsetof(Parameters, pCreateInfos), packed_pCreateInfos.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_array(parameters.pPipelines, parameters.createInfoCount, stream, payload_offset + offsetof(Parameters, pPipelines),
                                   packed_parameters->pPipelines);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = finalize_command_chunk(stream, range);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::unpack_parameters(SafeArrayView<std::uint8_t> & view, const Parameters ** parameters) {
    ...
    Parameters * packed_parameters = nullptr;
    VkResult     status            = unpack_command_chunk(view, CommandId::CreateGraphicsPipelines, 1, &packed_parameters);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = recover_command_pointer(packed_parameters->pCreateInfos, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    if (packed_parameters->pCreateInfos) {
        auto                                 child_view           = tail_view_from_pointer(view, packed_parameters->pCreateInfos);
        const VkGraphicsPipelineCreateInfo * ignored_pCreateInfos = nullptr;
        status = structure::unpack_array_VkGraphicsPipelineCreateInfo(child_view, (*packed_parameters).createInfoCount, &ignored_pCreateInfos);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
    }
    status = recover_command_pointer(packed_parameters->pAllocator, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = recover_command_pointer(packed_parameters->pPipelines, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::pack_response(CommandStream & stream, const Response & response) {
    ...
    Range      range;
    VkResult   status = append_command_chunk(stream, CommandId::CreateGraphicsPipelines, 1, response, range, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    const std::size_t payload_offset = range.offset + command_payload_offset<Response>();

    status =
        pack_output_array(response.pPipelines, response.createInfoCount, stream, payload_offset + offsetof(Response, pPipelines), packed_response->pPipelines);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = finalize_command_chunk(stream, range);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    return VK_SUCCESS;
...
VkResult Command::unpack_response(SafeArrayView<std::uint8_t> & view, const Response ** response) {
    Response * packed_response = nullptr;
    VkResult   status          = unpack_command_chunk(view, CommandId::CreateGraphicsPipelines, 1, &packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }

    status = recover_command_pointer(packed_response->pPipelines, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
