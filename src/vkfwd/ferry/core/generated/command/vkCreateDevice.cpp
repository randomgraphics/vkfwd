...
VkResult pack_into_blob(const Parameters & parameters, ParameterPacket * packet) {
    *packet            = ParameterPacket {};
    ...
    VkResult                status = structure::pack_VkDeviceCreateInfo(parameters.pCreateInfo, &blob, &create_info);
    ...
    header.command_id = static_cast<std::uint32_t>(CommandId::CreateDevice);
...
VkResult Command::unpack_parameters(const ParameterPacket & packet, Parameters * parameters) {
    ...
    if (!header || header->command_id != static_cast<std::uint32_t>(CommandId::CreateDevice) || !packed_parameters ||
        header->command_revision != kCommandRevision) {
