...
VkResult Command::pack_parameters(Blob & blob, const Parameters & parameters) {
    ...
    if constexpr (Hooks::before_pack_enabled) {
        ...
        CommandChunk chunk;
        VkResult     status = append_command_chunk(blob, CommandId::CreateDevice, 1, hook_parameters, chunk, packed_parameters);
        ...
        status = structure::pack_VkDeviceCreateInfo(hook_parameters.pCreateInfo, blob, packed_pCreateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_pointer(hook_parameters.pDevice, blob, payload_offset + offsetof(Parameters, pDevice), packed_parameters->pDevice);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        Parameters * packed_parameters = nullptr;
        CommandChunk chunk;
        VkResult     status = append_command_chunk(blob, CommandId::CreateDevice, 1, parameters, chunk, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = chunk.command_offset + command_payload_offset<Parameters>();

        structure::PackedStruct packed_pCreateInfo;
        status = structure::pack_VkDeviceCreateInfo(parameters.pCreateInfo, blob, packed_pCreateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pCreateInfo, payload_offset + offsetof(Parameters, pCreateInfo), packed_pCreateInfo.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_pointer(parameters.pDevice, blob, payload_offset + offsetof(Parameters, pDevice), packed_parameters->pDevice);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = finalize_command_chunk(blob, chunk);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::pack_response(Blob & blob, const Response & response) {
    ...
    CommandChunk chunk;
    VkResult     status = append_command_chunk(blob, CommandId::CreateDevice, 1, response, chunk, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    const std::size_t payload_offset = chunk.command_offset + command_payload_offset<Response>();

    status = pack_output_pointer(response.pDevice, blob, payload_offset + offsetof(Response, pDevice), packed_response->pDevice);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = finalize_command_chunk(blob, chunk);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    return VK_SUCCESS;
