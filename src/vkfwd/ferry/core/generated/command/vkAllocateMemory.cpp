...
namespace vkfwd::generated::commands::vkAllocateMemory {
namespace {
...
VkResult Command::pack_parameters(CommandStream & stream, const Parameters & parameters) {
    using Hooks = ::vkfwd::manual::CommandHooks<CommandId::AllocateMemory>;
    if constexpr (Hooks::before_pack_enabled) {
        ...
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::AllocateMemory, 1, hook_parameters, range, packed_parameters);
        ...
        structure::PackedStruct packed_pAllocateInfo;
        status = structure::pack_VkMemoryAllocateInfo(hook_parameters.pAllocateInfo, stream, packed_pAllocateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pAllocateInfo, payload_offset + offsetof(Parameters, pAllocateInfo), packed_pAllocateInfo.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_pointer(hook_parameters.pMemory, stream, payload_offset + offsetof(Parameters, pMemory), packed_parameters->pMemory);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        Parameters * packed_parameters = nullptr;
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::AllocateMemory, 1, parameters, range, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = range.offset + command_payload_offset<Parameters>();

        structure::PackedStruct packed_pAllocateInfo;
        status = structure::pack_VkMemoryAllocateInfo(parameters.pAllocateInfo, stream, packed_pAllocateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pAllocateInfo, payload_offset + offsetof(Parameters, pAllocateInfo), packed_pAllocateInfo.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_pointer(parameters.pMemory, stream, payload_offset + offsetof(Parameters, pMemory), packed_parameters->pMemory);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = finalize_command_chunk(stream, range);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::unpack_parameters(SafeArrayView<std::uint8_t> & view, const Parameters ** parameters) {
    ...
    Parameters * packed_parameters = nullptr;
    VkResult     status            = unpack_command_chunk(view, CommandId::AllocateMemory, 1, &packed_parameters);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = recover_command_pointer(packed_parameters->pAllocateInfo, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    if (packed_parameters->pAllocateInfo) {
        auto                         child_view            = tail_view_from_pointer(view, packed_parameters->pAllocateInfo);
        const VkMemoryAllocateInfo * ignored_pAllocateInfo = nullptr;
        status                                             = structure::unpack_VkMemoryAllocateInfo(child_view, &ignored_pAllocateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
    }
    status = recover_command_pointer(packed_parameters->pAllocator, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = recover_command_pointer(packed_parameters->pMemory, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::pack_response(CommandStream & stream, const Response & response) {
    ...
    Range      range;
    VkResult   status = append_command_chunk(stream, CommandId::AllocateMemory, 1, response, range, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    const std::size_t payload_offset = range.offset + command_payload_offset<Response>();

    status = pack_output_pointer(response.pMemory, stream, payload_offset + offsetof(Response, pMemory), packed_response->pMemory);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = finalize_command_chunk(stream, range);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    return VK_SUCCESS;
...
VkResult Command::unpack_response(SafeArrayView<std::uint8_t> & view, const Response ** response) {
    Response * packed_response = nullptr;
    VkResult   status          = unpack_command_chunk(view, CommandId::AllocateMemory, 1, &packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }

    status = recover_command_pointer(packed_response->pMemory, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
