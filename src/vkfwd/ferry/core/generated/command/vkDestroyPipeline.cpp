...
namespace vkfwd::generated::commands::vkDestroyPipeline {
namespace {
...
VkResult Command::pack_parameters(CommandStream & stream, const Parameters & parameters) {
    using Hooks = ::vkfwd::manual::CommandHooks<CommandId::DestroyPipeline>;
    if constexpr (Hooks::before_pack_enabled) {
        ...
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::DestroyPipeline, 1, hook_parameters, range, packed_parameters);
        ...
        Parameters * packed_parameters = nullptr;
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::DestroyPipeline, 1, parameters, range, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = range.offset + command_payload_offset<Parameters>();
...
VkResult Command::unpack_parameters(SafeArrayView<std::uint8_t> & view, const Parameters ** parameters) {
    ...
    Parameters * packed_parameters = nullptr;
    VkResult     status            = unpack_command_chunk(view, CommandId::DestroyPipeline, 1, &packed_parameters);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
