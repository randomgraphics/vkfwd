...
namespace vkfwd::generated::commands::vkEnumerateInstanceVersion {
namespace {
...
VkResult Command::pack_parameters(CommandStream & stream, const Parameters & parameters) {
    using Hooks = ::vkfwd::manual::CommandHooks<CommandId::EnumerateInstanceVersion>;
    if constexpr (Hooks::before_pack_enabled) {
        ...
        CommandChunk chunk;
        VkResult     status = append_command_chunk(stream, CommandId::EnumerateInstanceVersion, 1, hook_parameters, chunk, packed_parameters);
        ...
        status = pack_output_pointer(hook_parameters.pApiVersion, stream, payload_offset + offsetof(Parameters, pApiVersion), packed_parameters->pApiVersion);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        Parameters * packed_parameters = nullptr;
        CommandChunk chunk;
        VkResult     status = append_command_chunk(stream, CommandId::EnumerateInstanceVersion, 1, parameters, chunk, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = chunk.command_offset + command_payload_offset<Parameters>();

        status = pack_output_pointer(parameters.pApiVersion, stream, payload_offset + offsetof(Parameters, pApiVersion), packed_parameters->pApiVersion);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = finalize_command_chunk(stream, chunk);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::unpack_parameters(SafeArrayView<std::uint8_t> & view, const Parameters ** parameters) {
    ...
    Parameters * packed_parameters = nullptr;
    VkResult     status            = unpack_command_chunk(view, CommandId::EnumerateInstanceVersion, 1, &packed_parameters);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = recover_command_pointer(packed_parameters->pApiVersion, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::pack_response(CommandStream & stream, const Response & response) {
    ...
    CommandChunk chunk;
    VkResult     status = append_command_chunk(stream, CommandId::EnumerateInstanceVersion, 1, response, chunk, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    const std::size_t payload_offset = chunk.command_offset + command_payload_offset<Response>();

    status = pack_output_pointer(response.pApiVersion, stream, payload_offset + offsetof(Response, pApiVersion), packed_response->pApiVersion);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = finalize_command_chunk(stream, chunk);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    return VK_SUCCESS;
...
VkResult Command::unpack_response(SafeArrayView<std::uint8_t> & view, const Response ** response) {
    Response * packed_response = nullptr;
    VkResult   status          = unpack_command_chunk(view, CommandId::EnumerateInstanceVersion, 1, &packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }

    status = recover_command_pointer(packed_response->pApiVersion, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
