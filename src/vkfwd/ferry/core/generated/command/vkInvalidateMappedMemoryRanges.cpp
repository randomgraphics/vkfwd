...
namespace vkfwd::generated::commands::vkInvalidateMappedMemoryRanges {
namespace {
...
VkResult Command::pack_parameters(CommandStream & stream, const Parameters & parameters) {
    using Hooks = ::vkfwd::manual::CommandHooks<CommandId::InvalidateMappedMemoryRanges>;
    if constexpr (Hooks::before_pack_enabled) {
        ...
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::InvalidateMappedMemoryRanges, 1, hook_parameters, range, packed_parameters);
        ...
        structure::PackedStruct packed_pMemoryRanges;
        status = structure::pack_array_VkMappedMemoryRange(hook_parameters.pMemoryRanges, hook_parameters.memoryRangeCount, stream, packed_pMemoryRanges);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pMemoryRanges, payload_offset + offsetof(Parameters, pMemoryRanges), packed_pMemoryRanges.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        Parameters * packed_parameters = nullptr;
        Range        range;
        VkResult     status = append_command_chunk(stream, CommandId::InvalidateMappedMemoryRanges, 1, parameters, range, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = range.offset + command_payload_offset<Parameters>();

        structure::PackedStruct packed_pMemoryRanges;
        status = structure::pack_array_VkMappedMemoryRange(parameters.pMemoryRanges, parameters.memoryRangeCount, stream, packed_pMemoryRanges);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pMemoryRanges, payload_offset + offsetof(Parameters, pMemoryRanges), packed_pMemoryRanges.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = finalize_command_chunk(stream, range);
...
VkResult Command::unpack_parameters(SafeArrayView<std::uint8_t> & view, const Parameters ** parameters) {
    ...
    Parameters * packed_parameters = nullptr;
    VkResult     status            = unpack_command_chunk(view, CommandId::InvalidateMappedMemoryRanges, 1, &packed_parameters);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = recover_command_pointer(packed_parameters->pMemoryRanges, view);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    if (packed_parameters->pMemoryRanges) {
        auto                        child_view            = tail_view_from_pointer(view, packed_parameters->pMemoryRanges);
        const VkMappedMemoryRange * ignored_pMemoryRanges = nullptr;
        status = structure::unpack_array_VkMappedMemoryRange(child_view, (*packed_parameters).memoryRangeCount, &ignored_pMemoryRanges);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
    }
...
VkResult Command::pack_response(CommandStream & stream, const Response & response) {
    ...
    Range      range;
    VkResult   status = append_command_chunk(stream, CommandId::InvalidateMappedMemoryRanges, 1, response, range, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::unpack_response(SafeArrayView<std::uint8_t> & view, const Response ** response) {
    Response * packed_response = nullptr;
    VkResult   status          = unpack_command_chunk(view, CommandId::InvalidateMappedMemoryRanges, 1, &packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
