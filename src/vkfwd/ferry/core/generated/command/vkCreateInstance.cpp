...
VkResult pack_allocator(const VkAllocationCallbacks * allocator, Blob & blob, std::size_t pointer_slot_offset, const VkAllocationCallbacks *& pointer_slot) {
    (void) allocator;
    (void) blob;
...
VkResult Command::pack_parameters(Blob & blob, const Parameters & parameters) {
    ...
    if constexpr (Hooks::before_pack_enabled) {
        ...
        CommandChunk chunk;
        VkResult     status = append_command_chunk(blob, CommandId::CreateInstance, 1, hook_parameters, chunk, packed_parameters);
        ...
        status = structure::pack_VkInstanceCreateInfo(hook_parameters.pCreateInfo, blob, packed_pCreateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_allocator(hook_parameters.pAllocator, blob, payload_offset + offsetof(Parameters, pAllocator), packed_parameters->pAllocator);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_pointer(hook_parameters.pInstance, blob, payload_offset + offsetof(Parameters, pInstance), packed_parameters->pInstance);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        ...
        Parameters * packed_parameters = nullptr;
        CommandChunk chunk;
        VkResult     status = append_command_chunk(blob, CommandId::CreateInstance, 1, parameters, chunk, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = chunk.command_offset + command_payload_offset<Parameters>();

        structure::PackedStruct packed_pCreateInfo;
        status = structure::pack_VkInstanceCreateInfo(parameters.pCreateInfo, blob, packed_pCreateInfo);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = patch_command_pointer(packed_parameters->pCreateInfo, payload_offset + offsetof(Parameters, pCreateInfo), packed_pCreateInfo.offset);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_allocator(parameters.pAllocator, blob, payload_offset + offsetof(Parameters, pAllocator), packed_parameters->pAllocator);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = pack_output_pointer(parameters.pInstance, blob, payload_offset + offsetof(Parameters, pInstance), packed_parameters->pInstance);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        status = finalize_command_chunk(blob, chunk);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
...
VkResult Command::pack_response(Blob & blob, const Response & response) {
    Response *   packed_response = nullptr;
    CommandChunk chunk;
    VkResult     status = append_command_chunk(blob, CommandId::CreateInstance, 1, response, chunk, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    const std::size_t payload_offset = chunk.command_offset + command_payload_offset<Response>();

    status = pack_output_pointer(response.pInstance, blob, payload_offset + offsetof(Response, pInstance), packed_response->pInstance);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    status = finalize_command_chunk(blob, chunk);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
    return VK_SUCCESS;
