...
VkResult pack_allocator(const VkAllocationCallbacks * allocator, Blob & blob, std::size_t command_offset, const VkAllocationCallbacks *& pointer_slot) {
    ...
    try {
        std::size_t target      = 0;
        auto        destination = blob.grow<VkAllocationCallbacks>(1, alignof(VkAllocationCallbacks), &target);
        if (!destination.set(0, *allocator)) [[unlikely]] {
            VKFWD_LOG_ERROR("vkfwd ferry command pack failed: could not copy vkCreateInstance allocator callbacks into blob");
            return VK_ERROR_UNKNOWN;
        }
        return patch_command_pointer(pointer_slot, command_offset, target);
    } catch (const std::bad_alloc &) {
        VKFWD_LOG_ERROR("vkfwd ferry command pack failed: out of host memory while copying vkCreateInstance allocator callbacks");
...
VkResult pack_into_blob(Blob & blob, const Parameters & parameters, ParameterPacket & packet) {
    ...
    VkResult     status            = append_command_chunk(blob, CommandId::CreateInstance, kCommandRevision, parameters, packet, packed_parameters);
