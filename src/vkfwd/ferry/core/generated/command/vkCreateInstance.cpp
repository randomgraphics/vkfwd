...
VkResult Command::pack_parameters(CommandStream & stream, const Parameters & parameters) {
    ...
    if constexpr (Hooks::before_pack_enabled) {
        ...
        CommandChunk chunk;
        VkResult     status = append_command_chunk(stream, CommandId::CreateInstance, 1, hook_parameters, chunk, packed_parameters);
        ...
        Parameters * packed_parameters = nullptr;
        CommandChunk chunk;
        VkResult     status = append_command_chunk(stream, CommandId::CreateInstance, 1, parameters, chunk, packed_parameters);
        if (status != VK_SUCCESS) [[unlikely]] { return status; }
        const std::size_t payload_offset = chunk.command_offset + command_payload_offset<Parameters>();
...
VkResult Command::pack_response(CommandStream & stream, const Response & response) {
    ...
    CommandChunk chunk;
    VkResult     status = append_command_chunk(stream, CommandId::CreateInstance, 1, response, chunk, packed_response);
    if (status != VK_SUCCESS) [[unlikely]] { return status; }
