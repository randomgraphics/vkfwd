...
Blob::Blob(std::size_t chunk_size)
    : chunk_size_(std::max(chunk_size, kMinimumChunkSize)) {}
...
SafeArrayView<std::uint8_t> Blob::grow(std::size_t size,
  ...
  const auto offset = static_cast<std::size_t>(
      static_cast<std::byte*>(aligned) - chunk.data.get());
  chunk.used = checked_add(offset, size);
  size_ = checked_add(size_, size);
  return SafeArrayView<std::uint8_t>(
      size, reinterpret_cast<std::uint8_t*>(aligned));
}
...
Blob::Chunk Blob::allocate_chunk(std::size_t capacity,
                                 std::size_t alignment) {
  alignment = normalize_alignment(alignment);
...
Blob::Chunk& Blob::ensure_chunk(std::size_t size, std::size_t alignment) {
  if (!chunks_.empty()) {
    ...
    if (chunk.alignment >= alignment) {
      void* cursor = chunk.data.get() + chunk.used;
      std::size_t available = chunk.allocation_size - chunk.used;
      if (std::align(alignment, size, cursor, available)) {
        return chunk;
      }
    }
  }
