...
public:
  ...
  // serialization format and handoff target without changing the Vulkan layer
  ...
  void set_sink(std::unique_ptr<ForwardingSink> sink);
  ...
  // handoff. Today the record is minimal; the invariant for the real
  // implementation is that all borrowed Vulkan parameter memory has already
  // been converted into replayable owned data before this function returns.
...
private:
  ...
  std::unique_ptr<ForwardingSink> sink_;
};
